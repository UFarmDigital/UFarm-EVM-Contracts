...
import {IController, Controller} from "./Controller.sol";
...
contract OneInchV5Controller is Controller, NZGuard, UFarmErrors, ReentrancyGuard {
     ...
     */
    function delegated1InchMultiSwap(bytes[] calldata _multiData) external checkDelegateCall nonReentrant {
        uint256 swapsCount = _multiData.length;
        if (swapsCount == 0) revert EmptySwapPath();

        (address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut) = _run1InchSwap(_multiData[0]);
    ...
    function _run1InchSwap(
        ...
        if (method == IAggregationRouterV5.unoswapTo.selector) {
            ...
            for (uint256 i; i < pools.length; ++i) {
                ...
                (address token0, address token1) = (IUniswapV2Pair(v2Pool).token0(), IUniswapV2Pair(v2Pool).token1());
            ...
            for (uint256 i; i < length; ++i) {
                ...
                (address token0, address token1) = (IUniswapV3Pool(v2Pool).token0(), IUniswapV3Pool(v2Pool).token1());
                if (i > 0) {
                    // get dst token
            ...
            (amountIn, amountOut, pools) = abi.decode(_data.slice(4, _data.length - 4), (uint256, uint256, uint256[]));
    ...
    function _afterSwapExecuted(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut) internal {
        emit SwapOneInchV5(tokenIn, tokenOut, amountIn, amountOut, PROTOCOL());
    }

    function _onlyIfTokenAllowed(address token) internal view {
        if (
            IPoolWhitelist(address(this)).isTokenAllowed(token) == false &&
