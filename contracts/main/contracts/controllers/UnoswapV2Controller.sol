 ...
 */
abstract contract UnoswapV2Controller is IUnoswapV2Controller, Controller, NZGuard, UFarmErrors, ReentrancyGuard {
     ...
     */
    function PROTOCOL() public pure virtual override(Controller, IUnoswapV2Controller) returns (bytes32);
     ...
     */
    function delegateSwapExactTokensForTokens(bytes calldata _data) external checkDelegateCall nonReentrant {
        ...
        (address tokenA, address tokenB) = (reversed ? tokenOut : tokenIn, reversed ? tokenIn : tokenOut);
        ...
        for (uint256 i; i < pathLength - 2; ++i) {
            ...
            amountOut = getAmountOutReserves(amountOut, reversed ? reserve1 : reserve0, reversed ? reserve0 : reserve1);
    ...
    function delegatedAddLiquidity(bytes calldata _data) external checkDelegateCall nonReentrant {
        ...
        emit LiquidityAddedUnoV2(
            tokenA,
            tokenB,
            reversed ? amountB : amountA,
            reversed ? amountA : amountB,
            liquidity,
            pair,
            PROTOCOL()
        );
    ...
    function delegatedRemoveLiquidity(bytes calldata _data) external checkDelegateCall nonReentrant {
        ...
        emit LiquidityRemovedUnoV2(tokenA, tokenB, amountA, amountB, rlArgs.liquidity, pair, _target, PROTOCOL());
    }
    ...
    function encodePartialWithdrawalERC20(
        ...
        encodedTxs[0] = abi.encodeCall(UnoswapV2Controller.delegatedRemoveLiquidity, abi.encode(rlArgs));
    }
        ...
        returns (uint256 amountB, uint256 desiredLiquidity, uint256 totalLiquidity, address pair)
    {
        ...
        try IUniswapV2Pair(pair).getReserves() returns (uint112 reserveA, uint112 reserveB, uint32) {
            ...
            amountB = _quote(amountADesired, reversed ? reserveB : reserveA, reversed ? reserveA : reserveB);
    ...
    function quoteExactTokenAmounts(
        address lpToken,
        uint256 balance
    ) external view returns (address tokenA, address tokenB, uint256 amountA, uint256 amountB, uint256 totalLiquidity) {
        ...
        try _pair.getReserves() returns (uint112 reserveA, uint112 reserveB, uint32) {
            ...
            (amountA, amountB) = computeLiquidityValueWithFee(reserveA, reserveB, totalLiquidity, balance, true, klast);
        } catch {
            revert INVALID_PAIR();
        }
    }
    ...
    function _quoteExactLiquidityAmounts(
        ...
        try IUniswapV2Pair(pair).getReserves() returns (uint112 reserveA, uint112 reserveB, uint32) {
            ...
            if (amountBOptimal <= amountBDesired) {
                ...
                if (amountAOptimal < amountAMin || amountAOptimal > amountADesired) revert INSUFFICIENT_A_AMOUNT();
    ...
    function _getReserves(address tokenA, address tokenB) private view returns (uint256 reserveA, uint256 reserveB) {
        ...
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(pairForSorted(tokenA, tokenB)).getReserves();
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path) private view returns (uint256[] memory amounts) {
