    ...
    UFarmOwnableUUPSBeacon
{
     ...
     */
    event InvitationAccepted(address indexed inviter, address indexed invitee, bytes32 indexed msgHash);
    ...
    function verifyInvitation(
        ...
        if (invitation.deadline < block.timestamp) revert InvitationExpired(invitation.deadline, block.timestamp);
        ...
        returns (address pool, address poolAdmin)
    {
        ...
        IUFarmPool.CreationSettingsWithLinks memory fullSettings = IUFarmPool.CreationSettingsWithLinks({
            params: _settings,
            ufarmCore: ufarmCore,
            ufarmFund: address(this)
        });
        (pool, poolAdmin) = IPoolFactory(IUFarmCore(ufarmCore).poolFactory()).createPool(fullSettings, salt);
        ...
        poolManagementAllowed
    {
        ...
        IUFarmPool(_pool).deposit(_amount, IUFarmPool.ClientVerification({signature: "", validTill: 0, tier: 0}));
    }
        ...
        poolManagementAllowed
    {
        ...
        IUFarmPool(_request.body.poolAddr).withdrawByFund(_request, bearerToken);
    }
    ...
    function withdrawAsset(
        ...
        ownerOrHaveTwoPermissions(uint8(Permissions.Fund.Member), uint8(Permissions.Fund.ManageFund))
    {
        if (isPool[_token]) revert WrongAsset();
        if (_token == address(0)) {
    ...
    function _canUpdatePermissions(address _updater, address _updatee, uint256 _newPermissionsMask) private view {
        ...
        if (_isPermissionDiff(uint8(Permissions.Fund.Owner), currentUpdateeMask, _newPermissionsMask)) {
            ...
            if (!_hasPermission(_updater, uint8(Permissions.Fund.Owner))) {
                ...
                _checkForPermissions(
                    _updater,
                    _twoPermissionsToMask(uint8(Permissions.Fund.Member), uint8(Permissions.Fund.UpdateFundPermissions))
                );
    ...
    function _hashInvitation(FundMemberInvitation memory invitation) private pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    keccak256(bytes("FundMemberInvitation(address invitee,uint256 permissionsMask,uint256 deadline)")),
                    invitation.invitee,
