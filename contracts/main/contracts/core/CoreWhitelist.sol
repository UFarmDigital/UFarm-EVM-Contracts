...
abstract contract CoreWhitelist is ICoreWhitelist, NZGuard, UFarmErrors {
    ...
    EnumerableSet.AddressSet private __valueTokens;

    event TokenAdded(AssetWithPriceFeed assetInfo);
    event TokenRemoved(address indexed token);
    event ValueTokenAdded(address indexed token);
    event ValueTokenRemoved(address indexed token);
    ...
    error ValueTokenAlreadyAdded(address token);
    error ValueTokenNotRemoved(address token);
    error ProtocolNotAllowed(bytes32 protocol);

    /**
     * @inheritdoc ICoreWhitelist
     */
    function tokenInfo(address token) external view override returns (AssetWithPriceFeed memory info) {
     ...
     * @inheritdoc ICoreWhitelist
     */
    function getWhitelistedProtocols() external view virtual override returns (bytes32[] memory protocols) {
    ...
    function _getWhitelistedValueTokens() internal view returns (address[] memory valueTokens) {
        return __valueTokens.values();
    }
     ...
     */
    function isValueTokenWhitelisted(address token) public view returns (bool) {
        return __valueTokens.contains(token);
    }

    function _whitelistValueTokens(address[] memory tokens) internal {
        uint256 length = tokens.length;
        _nonEmptyArray(length);
        address token;
        for (uint256 i; i < length; ++i) {
            token = tokens[i];
            if (__valueTokens.add(token)) {
                emit ValueTokenAdded(token);
            } else revert ValueTokenAlreadyAdded(token);
        }
    }

    function _blacklistValueTokens(address[] memory tokens) internal {
        uint256 length = tokens.length;
        _nonEmptyArray(length);
        address token;
        for (uint256 i; i < length; ++i) {
            token = tokens[i];
            if (__valueTokens.remove(token)) {
                emit ValueTokenRemoved(token);
            } else revert ValueTokenNotRemoved(token);
        }
    }
    ...
    function _updateProtocol(bytes32 _protocol, address _controller) internal nonZeroBytes32(_protocol) {
