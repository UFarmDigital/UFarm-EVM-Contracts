...
library SafeOPS {
    ...
    function _safeCall(address _to, bytes memory _data) internal returns (bool success, bytes memory result) {
                    ...
                    uint(
                        keccak256(
                            abi.encodePacked(
                                ...
                                _salt,
                                keccak256(
                                    abi.encodePacked(type(BeaconProxy).creationCode, abi.encode(_implBeacon, _initCall))
                                )
                            )
                        )
                    )
